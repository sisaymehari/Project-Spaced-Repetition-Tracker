...
function displayUserAgenda(userId) {
  ...
  if (!userData || userData.length === 0) {
    agendaContent.innerHTML =
      '<div class="no-agenda">No topics to revise yet. Add a topic below to get started!</div>';
    return;
  }
  ...
  if (futureRevisions.length === 0) {
    agendaContent.innerHTML =
      '<div class="no-agenda">No upcoming revisions. All topics are up to date!</div>';
    return;
  }
...
function handleFormSubmit(event) {
  ...
  try {
    ...
    announceToScreenReader(
      `Topic "${topicName}" added successfully with ${revisionDates.length} revision dates.`
    );
  } catch (error) {
